...
> [!tip]
> You are *strongly* encouraged to compile your own version of the application, rather than using the pre-compiled version. There is both an optimized and PGO optimized recipe that can be invoked by `make optimized` and `make pgo` respectively. For reference, an unoptimized version runs in $12749$ milliseconds, optimized in $1281$, and PGO in $1044$, or a speed by a factor of $10$ for optimized, and a 20% speedup on top of that for PGO. There are no dependencies outside of the standard library and compiler, so if you have `g++` and `libc`, so you should be able to compile it using `make`. All examples henceforth will use the name `main` as the program name, but if you are using the precompiled version you should change that to `main_pc`. See [[ISEC 611/MG-GA/BUILDING|BUILDING]] for details
